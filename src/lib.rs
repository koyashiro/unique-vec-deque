...
mod tests {
    ...
    #[test]
    fn unique_vec_deque_default() {
        let deque = UniqueVecDeque::<u8>::default();
        assert_eq!(deque.deque.len(), 0);
        assert_eq!(deque.deque.capacity(), 0);
        assert_eq!(deque.set.len(), 0);
        assert_eq!(deque.set.capacity(), 0);
    }

    #[test]
    fn unique_vec_deque_clone() {
        let orig = UniqueVecDeque {
            deque: [0, 1, 2, 3].into(),
            set: hashset! { 0, 1, 2, 3 },
        };
    ...
    #[test]
    fn unique_vec_deque_rotate_left() {
        let mut deque = UniqueVecDeque {
            deque: [0, 1, 2, 3].into(),
            set: hashset! { 0, 1, 2, 3 },
        };
        deque.rotate_left(3);
        assert_eq!(deque.deque, [3, 0, 1, 2]);
    }

    #[test]
    fn unique_vec_deque_rotate_right() {
        let mut deque = UniqueVecDeque {
            deque: [0, 1, 2, 3].into(),
            set: hashset! { 0, 1, 2, 3 },
        };
        deque.rotate_right(3);
        assert_eq!(deque.deque, [1, 2, 3, 0]);
    }

    #[test]
    fn unique_vec_deque_binary_search() {
        let deque = UniqueVecDeque {
            deque: [0, 1, 2, 3].into(),
            set: hashset! { 0, 1, 2, 3 },
        };
        assert_eq!(deque.binary_search(&3), Ok(3));
        assert_eq!(deque.binary_search(&4), Err(4));
    }

    #[test]
    fn unique_vec_deque_binary_search_by() {
        let deque = UniqueVecDeque {
            deque: [0, 1, 2, 3].into(),
            set: hashset! { 0, 1, 2, 3 },
        };
        assert_eq!(deque.binary_search_by(|elem| elem.cmp(&3)), Ok(3));
        assert_eq!(deque.binary_search_by(|elem| elem.cmp(&4)), Err(4));
    }

    #[test]
    fn unique_vec_deque_binary_search_by_key() {
        let deque = UniqueVecDeque {
            deque: [(0, 1), (2, 3), (4, 5), (6, 7)].into(),
            set: hashset! { (0, 1), (2, 3), (3, 4), (5, 6) },
        };
        assert_eq!(deque.binary_search_by_key(&2, |&(a, _b)| a), Ok(1));
        assert_eq!(deque.binary_search_by_key(&7, |&(a, _b)| a), Err(4));
    }
    ...
    #[test]
    fn unique_vec_deque_partial_cmp() {
        let deque = UniqueVecDeque {
            deque: [0, 1, 2, 3].into(),
            set: hashset! { 0, 1, 2, 3 },
        };
        ...
        assert_eq!(
            deque.partial_cmp(&UniqueVecDeque {
                deque: [1, 2, 3, 4].into(),
                set: hashset! { 1, 2, 3, 4 },
            }),
            Some(Ordering::Less)
